...
// ===== ZAMANLAYICI =====
// startTimer aşağıda tanımlandı (exitAutoPlay'in yanında)
                ...
                try {
                    ...
                    if(isMultiplayer) {
                        ...
                        if(ok){
                            ...
                            window.updateStats();
                            window.checkWinner();
                            // Oyun bittiyse (kazanan ekranı açıldıysa) turu bitirmeye çalışma
                            const overlay = document.getElementById('winOverlay');
                            const gameOver = overlay && overlay.style.display === 'flex';
                            if(!gameOver && player.cardsPlayed>=player.maxPlays) endTurn();
                        }
                    }
                } finally {
                    isAnimating=false;
...
function buildCardElement(card, index) {
  ...
  const activeSkin = window.userProfile?.activeSkin;
  if (activeSkin && activeSkin !== 'default') div.classList.add('card-skin-' + activeSkin);
  // Elimdeki kartlar her zaman benim — satın aldığım kart efekti burada da görünsün
  const activeEffect = window.userProfile?.activeEffect;
  if (activeEffect) div.classList.add('hand-effect-' + activeEffect);
...
window.renderGameLayout = function() {
    ...
    // Mağazadan alınan masa temasını uygula
    container.className = container.className.replace(/\bbg-theme-\S+/g, '').trim();
    const activeBg = window.userProfile?.activeBg;
    if (activeBg && activeBg !== 'default') container.classList.add('bg-theme-' + activeBg);
    ...
    players.forEach((p,i)=>{
        ...
        const myFrame = isMe ? (window.userProfile?.activeFrame) : null;
        const frameClass = myFrame && myFrame !== 'none' ? ` frame-${myFrame}` : '';
        div.innerHTML=`
            <div class="seat-avatar-wrap">
              <div class="panel-avatar${frameClass}">${p.avatar||'👤'}</div>
              <div class="panel-hp-badge">❤️<span id="playerHp_${p.id}">${p.hp}</span></div>
              <div class="panel-shield-badge">🛡️<span id="playerBarricade_${p.id}">${p.barricade}</span></div>
            </div>
            <div class="panel-name">${p.name} ${isMe?'<span style="color:var(--gold);font-size:9px">(SEN)</span>':''} ${vipBadge}</div>
            <div class="panel-timer-row">
                <span id="playerTimer_${p.id}">30</span>
...
function executeCardAction(actorIdx,targetIdx,idx) {
    ...
    // Masanın ortasında göster — efekt sadece benim kartımsa uygulanır
    const meta = CARD_META[card] || {};
    const isMine = actorIdx === (isMultiplayer ? myPlayerId : 0);
    window.showCenterCard?.(meta.emoji || card.split(' ')[0], meta.shortName || card.replace(/[^\w\sğüşıöçĞÜŞİÖÇ]/gu,'').trim(), isMine);
    return true;
}
...
function botTurn() {
    ...
    function step(){
        if(actor.cardsPlayed<plays&&actor.hand.length>0&&actor.hp>0){
            ...
            window.checkWinner();
            const overlay = document.getElementById('winOverlay');
            if (overlay && overlay.style.display === 'flex') return; // oyun bitti, devam etme
            setTimeout(step,800);
...
window.exitAutoPlay = function() {
    ...
    // El kartlarını zorla yenile
    window.renderHand();
    // Sırayı sıfırla ve timer başlat
    if (!isMultiplayer && players[turnIdx] && !players[turnIdx].isBot) {
        window.startTimer();
    }
    // Multiplayer'da sıra bilgisi Firestore'dan gelir, timer Firestore snapshot'ta başlar
};

// Sorun 4: Multiplayer'da timer bitince bot devralmasın — sadece sırayı geç
window._multiplayerTurnTimeout = null;
...
window.startTimer = function() {
    ...
    window.timerId = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            clearInterval(window.timerId);
            if (isMultiplayer) {
                // Multiplayer: süre dolunca kart oynamadan sırayı geç
                log("⏰ Süren doldu, sıra geçiyor...");
                window.endTurnMultiplayer?.();
            } else if (players[turnIdx]?.isBot) {
                endTurn();
            } else {
                log("⏰ Süre doldu!");
                isAutoPlay = true;
...
window.initGame = function(mode) {
    ...
    if(mode==='pvb'){
        const me = makeLocal(0,window.userProfile?.name||"Oyuncu",window.userProfile?.avatar||"😎",false);
        me.vipLevel = window.userProfile?.vipLevel||0;
        players.push(me);
    ...
    setTimeout(()=>window.attachPanelClickListeners?.(), 150);
    window.renderHand();
    window.updateStats();
...
// Masanın ortasında oynanan kartı göster — Okey101 gibi
// Kart, turu bitiren clearCenterCards() çağrılana kadar masada kalır (sabit süreyle kaybolmaz)
// isMine: bu kartı satın aldığım efektle göstereyim mi? — sadece KENDİ oynadığım kartlarda true olmalı,
// rakip/bot kart oynadığında benim efektim onlara binmemeli (efekt kişiseldir, başkası görmez)
window.showCenterCard = function(cardEmoji, cardName, isMine=false) {
  const area = document.getElementById('centerPlayArea');
  if (!area) return;

  const el = document.createElement('div');
  const activeEffect = isMine ? window.userProfile?.activeEffect : null;
  el.className = 'center-played-card' + (activeEffect ? ` effect-${activeEffect}` : '');
  el.innerHTML = `<div class="cpc-emoji">${cardEmoji}</div><div class="cpc-name">${cardName||''}</div>`;
  area.appendChild(el);
};

// Tur sonunda masadaki kartları temizle
window.clearCenterCards = function() {
  const area = document.getElementById('centerPlayArea');
  if (!area) return;
