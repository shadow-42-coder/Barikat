...
function executeCardAction(actorIdx,targetIdx,idx) {
    ...
    window.showCenterCard?.(meta.emoji || card.split(' ')[0], meta.shortName || card.replace(/[^\w\sğüşıöçĞÜŞİÖÇ]/gu,'').trim());
    return true;
}
...
window.showCenterCard = function(cardEmoji, cardName, duration=2500) {
  ...
  // Süre sonunda fade out ile kaybol
  setTimeout(() => {
