...
// window.userProfile = {...} ataması (systems.js/social.js'de sıkça yapılıyor)
// yerel `userProfile` değişkenini de güncellesin — aksi halde updateAllUI() eski veriyi gösterir
...
function updateAllUI() {
  ...
  // Satın alınan çerçeveyi avatarlara uygula
  applyFrameClass(document.getElementById('profileAvatarBig'), p.activeFrame);
  applyFrameClass(document.getElementById('menuAvatar'), p.activeFrame);

  // VIP rozetini menü ve profil ekranında göster
  const vipBadgeHTML = window.getVipBadge ? window.getVipBadge(p.vipLevel||0) : '';
  const menuVipEl = document.getElementById('menuVipBadge');
  if (menuVipEl) { menuVipEl.innerHTML = vipBadgeHTML; menuVipEl.style.display = vipBadgeHTML ? 'inline-block' : 'none'; }
  const profileVipEl = document.getElementById('profileVipBadge');
  if (profileVipEl) { profileVipEl.innerHTML = vipBadgeHTML; profileVipEl.style.display = vipBadgeHTML ? 'inline-block' : 'none'; }
...
// Mağazadan alınan çerçeveyi bir avatar elemanına uygular (none/boş ise kaldırır)
function applyFrameClass(el, frameId) {
  if (!el) return;
  el.className = (el.className || '').replace(/\bframe-\S+/g, '').trim();
  if (frameId && frameId !== 'none') el.classList.add('frame-' + frameId);
}
window.applyFrameClass = applyFrameClass;
...
window.doJoinRoom = async function() {
  ...
  const newPlayer = makePlayerObj(_myPlayerId, userProfile.name, userProfile.avatar, false);
...
function makePlayerObj(id, name, avatar, isBot) {
  return {
    ...
    teamId: id, isBot,
    vipLevel: isBot ? 0 : (userProfile?.vipLevel||0),
...
function showLobby(roomId, pin, maxPlayers, players) {
  ...
  lobbyUnsubscribe = onSnapshot(gameRef, (snap) => {
    ...
    if (gd.swapRequest && gd.swapRequest.toId === _myPlayerId && gd.swapRequest.fromId !== _myPlayerId) {
    ...
    if (gd.gameStarted) {
      ...
      window.startGameMultiplayer(roomId, gd.gameMode, _myPlayerId);
    }
  });
}
...
async function sitOnSeat(targetSeatIdx) {
  ...
  const gameRef = doc(db,'games',_currentRoomId);
  const snap = await getDoc(gameRef);
  if (!snap.exists()) return;
  const gd = snap.data();

  // Zaten o slottaysa bir şey yapma
  if (_myPlayerId === targetSeatIdx) return;
  // O slot doluysa
...
window.respondSwap = async function(accepted) {
  ...
  if (!_currentRoomId || !pendingSwap) return;
  const gameRef = doc(db,'games',_currentRoomId);
...
function startLobbyTimer(roomId, maxPlayers) {
  ...
  lobbyTimerInterval = setInterval(async () => {
    ...
    if (t <= 0) {
      ...
      if (!newReady.includes(_myPlayerId)) newReady.push(_myPlayerId);
...
window.toggleReady = async function() {
  ...
  if (myReady) { if (!readyList.includes(_myPlayerId)) readyList.push(_myPlayerId); }
  else { readyList = readyList.filter(id => id !== _myPlayerId); }
  await updateDoc(gameRef, { readyPlayers: readyList });
};

// Oyunu başlat (sadece host)
window.startLobbyGame = async function() {
  if (!isHost || !_currentRoomId) return;
  const gameRef = doc(db,'games',_currentRoomId);
  const roomRef = doc(db,'rooms',_currentRoomId);
  const snap = await getDoc(gameRef);
  if (!snap.exists()) return;
  const gd = snap.data();
...
window.startGameMultiplayer = function(gameId, gameType, assignedPlayerId) {
  ...
  _myPlayerId = assignedPlayerId;
  window.isMultiplayer = true;
  window._lastLogSeenLen = undefined;
  ...
  window._gameUnsubscribe = onSnapshot(gameRef, (snap) => {
    ...
    // Yeni oynanan kartları masanın ortasında göster (herkes görsün — efekt sadece kart sahibinin kendi ekranında)
    const fullLog = gd.log || [];
    const prevLen = window._lastLogSeenLen ?? fullLog.length;
    const myName = gd.players.find(p => p && p.id === myPlayerId)?.name;
    if (fullLog.length > prevLen) {
      fullLog.slice(prevLen).forEach((entry, i) => {
        const m = /^(.+?) oynadı: (.+)$/.exec(entry);
        if (m) {
          const actorName = m[1];
          const cardName = m[2];
          const meta = window.CARD_META?.[cardName] || {};
          const isMine = actorName === myName;
          setTimeout(() => window.showCenterCard?.(meta.emoji || cardName.split(' ')[0], meta.shortName || cardName, isMine), i * 250);
        } else if (/turunu bitirdi\.$/.test(entry)) {
          setTimeout(() => window.clearCenterCards?.(), 1400);
        }
      });
    }
    window._lastLogSeenLen = fullLog.length;

    // Oyun yeni başladıysa (herkes tam HP) kazanan kontrolü yapma
    const allFullHp = activePlayers.every(p => p.hp >= 7);
    const isTeamMode = gameType === '2V2' || gameType === '2v2';
    const aliveTeams = new Set(alive.map(p => p.teamId));
    const gameOver = isTeamMode ? aliveTeams.size <= 1 : alive.length <= 1;
    if (!allFullHp && gameOver && activePlayers.length > 1) {
      ...
      if (overlay && overlay.style.display !== 'flex') {
        ...
        const iWon = isTeamMode
          ? alive.some(p => p.id === _myPlayerId)
          : (winner && winner.id === _myPlayerId);
        ...
        const opp = gd.players.find(p=>p&&p.id!==_myPlayerId)?.name||'Rakip';
    ...
    if (ti) {
      ti.textContent = gd.turnIdx === _myPlayerId ? '⚡ SENİN SIRAN' : `⏳ ${actor?.name||''}`;
      ti.style.color = gd.turnIdx === _myPlayerId ? 'var(--gold)' : 'var(--text2)';
    }

    if (gd.turnIdx === _myPlayerId) {
...
window.playCardMultiplayer = async function(actorIdx, targetIdx, cardIdx) {
  if (!_currentRoomId) return false;
  const gameRef = doc(db,'games',_currentRoomId);
  ...
  if (actor.cardsPlayed >= actor.maxPlays) {
    ...
    if (isHost && up[nextTurnIdx].isBot) {
      setTimeout(() => window.playBotTurnFirestore(nextTurnIdx, up, gd.deck, newLog, _currentRoomId), 1200);
    }
  }
...
function showScreen(id) {
  // Önceki ekranı hatırla — "Geri" butonu oyun ortasında ayarlara/profile girilse de
  // oyunu (screen-game) kaybetmeden geri dönebilsin
  const current = document.querySelector('.screen.active');
  if (current && current.id !== id) window._prevScreen = current.id;
...
// Profil/Ayarlar gibi ekranlardaki "← Geri" butonu — oyun içindeyken ayarlara girilirse oyuna geri döner
window.goBack = function() {
  showScreen(window._prevScreen || 'screen-menu');
};
...
window.applySetting = function(key, val) {
  ...
  if (key==='theme') {
    ...
    if (val && val !== 'dark') document.body.classList.add('theme-' + val);
    localStorage.setItem('barikat_theme', val);
  }
};

// Sayfa açılışında kayıtlı temayı uygula
document.addEventListener('DOMContentLoaded', function() {
  const savedTheme = localStorage.getItem('barikat_theme');
  if (savedTheme) {
    window.applySetting('theme', savedTheme);
    const sel = document.getElementById('stgTheme');
