...
// Oturum kalıcı olsun — tarayıcı kapansa da giriş kalsın
...
async function loadUserProfile(uid) {
  ...
  if (uid === DEV_UID) {
    ...
    // Firestore'a da yaz ki sayfa yenilenince tekrar 0 gelmesin
...
function showLobby(roomId, pin, maxPlayers, players) {
  ...
  lobbyUnsubscribe = onSnapshot(gameRef, (snap) => {
    ...
    // Yer değiştirme isteği bana mı geldi?
    if (gd.swapRequest && gd.swapRequest.toId === myPlayerId && gd.swapRequest.fromId !== myPlayerId) {
      ...
      if (!pendingSwap || pendingSwap.ts !== req.ts) {
        showSwapRequest(req);
      }
    } else if (!gd.swapRequest && document.getElementById('swapRequestModal').style.display !== 'none') {
      // İstek iptal edildi
...
window.startGameMultiplayer = function(gameId, gameType, assignedPlayerId) {
  ...
  window._gameUnsubscribe = onSnapshot(gameRef, (snap) => {
    ...
    // Kazanan kontrolü — sadece çalışan oyuncular için
    const alive = gd.players.filter(p => p && p.hp > 0);
    if (alive.length <= 1 && gd.players.length > 1) {
    ...
    // Log — sadece log kutusunu güncelle
    const la = document.getElementById('log');
    if (la) {
      const logs = (gd.log||[]).slice(-30);
      la.innerHTML = logs.map(m=>m+'<br>').join('');
