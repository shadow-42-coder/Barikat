  ...
  if (tab === 'gems') {
    content.innerHTML = `<div class="settings-group"><h3>💎 Elmas Al</h3>
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px">
        ${[{g:80,c:1500},{g:200,c:3500,bonus:'%17'},{g:500,c:8000,bonus:'%25'},{g:1000,c:14000,bonus:'%33'},{g:2500,c:30000,bonus:'%50',hot:true},{g:5000,c:50000,bonus:'%67',hot:true}]
          .map(i=>`<div class="shop-item" onclick="buyGems(${i.g},${i.c})">
            ${i.hot?'<div class="shop-badge">🔥 HOT</div>':''}
            ${i.bonus?`<div class="shop-badge" style="background:var(--green)">${i.bonus} bonus</div>`:''}
            <div class="shop-gem-icon">💎×${i.g}</div>
            <div class="shop-price">🪙 ${i.c.toLocaleString('tr-TR')}</div>
          </div>`).join('')}
      </div></div>`;
  }
  else if (tab === 'effects') {
    content.innerHTML = `<div class="settings-group"><h3>✨ Kart Efektleri</h3>
      <p style="font-size:12px;color:var(--text2);margin-bottom:12px">Kart oynarken çıkan görsel efektler — oyunu etkilemez</p>
      <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px">
        ${Object.entries(SHOP_EFFECTS).map(([id,e])=>{
          const isOwned = owned.includes(id);
          const isActive = p.activeEffect === id;
          return `<div class="shop-item ${isActive?'shop-item-active':''}" onclick="${isOwned?`activateEffect('${id}')`:`buyEffect('${id}')`}">
            ...
            ${isOwned
              ? `<div class="shop-price" style="color:${isActive?'var(--green)':'var(--text2)'}"> ${isActive?'✅ Aktif':'▶ Kullan'}</div>`
    ...
    const activeSkin = p.activeSkin || 'default';
    content.innerHTML = `<div class="settings-group"><h3>🃏 Kart Skin</h3>
      <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px">
        ${Object.entries(CARD_SKINS).map(([id,s])=>{
          const isOwned = id==='default'||(p.ownedSkins||[]).includes(id);
          const isActive = activeSkin === id;
          return `<div class="shop-item ${isActive?'shop-item-active':''}" onclick="${isOwned?`activateSkin('${id}')`:`buySkin('${id}')`}">
    ...
    const activeBg = p.activeBg || 'default';
    content.innerHTML = `<div class="settings-group"><h3>🎨 Masa Teması</h3>
      <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px">
        ${Object.entries(BG_THEMES).map(([id,s])=>{
          const isOwned = id==='default'||(p.ownedBgs||[]).includes(id);
          const isActive = activeBg === id;
          return `<div class="shop-item ${isActive?'shop-item-active':''}" onclick="${isOwned?`activateBg('${id}')`:`buyBg('${id}')`}">
            <div class="shop-icon">${s.preview}</div>
            <div class="shop-name">${s.name}</div>
            <div class="shop-desc">${s.desc||''}</div>
            ${isOwned
    ...
    const activeFrame = p.activeFrame || 'none';
    content.innerHTML = `<div class="settings-group"><h3>🖼️ Profil Çerçevesi</h3>
      <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px">
        ${Object.entries(FRAMES).map(([id,f])=>{
          const isOwned = id==='none'||(p.ownedFrames||[]).includes(id);
          const isActive = activeFrame === id;
          return `<div class="shop-item ${isActive?'shop-item-active':''}" onclick="${isOwned?`activateFrame('${id}')`:`buyFrame('${id}')`}">
            <div class="shop-icon">${f.preview}</div>
            <div class="shop-name">${f.name}</div>
            ${isOwned
              ? `<div class="shop-price" style="color:${isActive?'var(--green)':'var(--text2)'}"> ${isActive?'✅ Aktif':'▶ Seç'}</div>`
    ...
    content.innerHTML = `<div class="settings-group"><h3>👑 VIP Seviyeleri</h3>
      <p style="font-size:12px;color:var(--text2);margin-bottom:12px">Mevcut VIP: <strong style="color:var(--gold)">VIP ${currentVip}</strong></p>
      <div style="display:flex;flex-direction:column;gap:8px">
        ${VIP_LEVELS.filter(v=>v.level>0).map(v=>{
          const isOwned = currentVip >= v.level;
          const isNext = currentVip === v.level - 1;
          return `<div class="shop-item ${isOwned?'shop-item-active':''}" style="display:flex;align-items:center;gap:12px;text-align:left" onclick="${!isOwned?`buyVIP(${v.level})`:''}">
            <div class="shop-icon vip-badge vip-${Math.min(v.level,5)}" style="font-size:16px;min-width:60px;text-align:center">VIP ${v.level}</div>
            <div style="flex:1">
              <div class="shop-name">${v.name}</div>
              <div class="shop-desc">${v.perks.join(' · ')}</div>
            </div>
            ${isOwned
              ? '<div style="color:var(--green);font-weight:700;white-space:nowrap">✅ Sahip</div>'
...
window.activateEffect = async function(id) {
  ...
  showToast(`✅ Efekt aktif edildi!`);
  renderShopTab('effects');
};

// Skin satın al
window.buySkin = async function(id) {
  const s = CARD_SKINS[id];
  if (!s) return;
  const p = window.userProfile || {};
  const uid = window.auth?.currentUser?.uid;
  if (!uid) return;
  if ((p.gems||0) < s.cost) { showToast('❌ Yeterli elmas yok!'); return; }
  if (!confirm(`${s.name} için ${s.cost} 💎 harcansın mı?`)) return;
  const newOwned = [...(p.ownedSkins||[]), id];
  await (window._fb.updateDoc)((window._fb.doc)(window.db,'users',uid), {
    gems: (window._fb.increment)(-s.cost), ownedSkins: newOwned, activeSkin: id
  });
  window.userProfile = { ...p, gems:(p.gems||0)-s.cost, ownedSkins: newOwned, activeSkin: id };
  showToast(`✅ ${s.name} satın alındı!`);
  updateGemDisplays(); renderShopTab('skins');
};
window.activateSkin = async function(id) {
  const uid = window.auth?.currentUser?.uid; if (!uid) return;
  await (window._fb.updateDoc)((window._fb.doc)(window.db,'users',uid), { activeSkin: id });
  window.userProfile = { ...window.userProfile, activeSkin: id };
  showToast('✅ Skin aktif!'); renderShopTab('skins');
};

// BG satın al
window.buyBg = async function(id) {
  const s = BG_THEMES[id]; if (!s) return;
  const p = window.userProfile||{}; const uid = window.auth?.currentUser?.uid; if (!uid) return;
  if ((p.gems||0)<s.cost){showToast('❌ Yeterli elmas yok!');return;}
  if (!confirm(`${s.name} için ${s.cost} 💎?`)) return;
  const newOwned=[...(p.ownedBgs||[]),id];
  await (window._fb.updateDoc)((window._fb.doc)(window.db,'users',uid),{gems:(window._fb.increment)(-s.cost),ownedBgs:newOwned,activeBg:id});
  window.userProfile={...p,gems:(p.gems||0)-s.cost,ownedBgs:newOwned,activeBg:id};
  showToast(`✅ ${s.name}!`); updateGemDisplays(); renderShopTab('bg');
};
window.activateBg = async function(id) {
  const uid=window.auth?.currentUser?.uid; if(!uid)return;
  await (window._fb.updateDoc)((window._fb.doc)(window.db,'users',uid),{activeBg:id});
  window.userProfile={...window.userProfile,activeBg:id}; showToast('✅ Tema değişti!'); renderShopTab('bg');
};

// Frame satın al
window.buyFrame = async function(id) {
  const f=FRAMES[id]; if(!f)return;
  const p=window.userProfile||{}; const uid=window.auth?.currentUser?.uid; if(!uid)return;
  if((p.gems||0)<f.cost){showToast('❌ Yeterli elmas yok!');return;}
  if(!confirm(`${f.name} için ${f.cost} 💎?`))return;
  const newOwned=[...(p.ownedFrames||[]),id];
  await (window._fb.updateDoc)((window._fb.doc)(window.db,'users',uid),{gems:(window._fb.increment)(-f.cost),ownedFrames:newOwned,activeFrame:id});
  window.userProfile={...p,gems:(p.gems||0)-f.cost,ownedFrames:newOwned,activeFrame:id};
  showToast(`✅ ${f.name}!`); updateGemDisplays(); renderShopTab('frames');
};
window.activateFrame = async function(id) {
  const uid=window.auth?.currentUser?.uid; if(!uid)return;
  await (window._fb.updateDoc)((window._fb.doc)(window.db,'users',uid),{activeFrame:id});
  window.userProfile={...window.userProfile,activeFrame:id}; showToast('✅ Çerçeve değişti!'); renderShopTab('frames');
};

// Mağaza açılınca ilk sekmeyi göster
const origShowScreen = window.showScreen;
window.showScreen = function(id) {
  origShowScreen?.(id);
  if (id === 'screen-shop') {
    setTimeout(() => renderShopTab('gems'), 50);
  }
  if (id === 'screen-quests') {
...
// shop-item-active CSS eklentisi
const styleEl = document.createElement('style');
styleEl.textContent = '.shop-item-active { border-color: rgba(46,204,113,0.5) !important; background: rgba(46,204,113,0.06) !important; }';
