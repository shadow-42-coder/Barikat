...
// ===== YARDIMCILAR =====
// _fb() doğrudan çağrılmaz — her fonksiyon kendi içinde window._fb kontrol eder
function _getFb() {
  if (!window._fb) { console.warn('_fb henüz hazır değil'); return null; }
  return window._fb;
}
function _db() { return window._firebaseDb || window.db; }
function _uid() { return window._firebaseAuth?.currentUser?.uid || window.auth?.currentUser?.uid; }
...
window.showPlayerContextMenu = function(playerId, playerName, playerAvatar, event) {
  ...
  _clickedPlayer = { id: playerId, name: playerName, avatar: playerAvatar, x: event?.clientX, y: event?.clientY };
  ...
  menu.innerHTML = `
    ...
    <button class="ctx-btn" onclick="openGiftToPlayer('${playerId}','${playerName}',event)">🎁 Hediye Gönder</button>
...
window.viewPlayerProfile = async function(targetUid) {
  ...
  if (typeof targetUid === 'string' && targetUid.length > 10) {
    try {
      const fb = _getFb(); if (!fb) return;
      const snap = await fb.getDoc(fb.doc(_db(),'users',targetUid));
  ...
  window.applyFrameClass?.(modal.querySelector('#vpAvatar'), profile.activeFrame);
  ...
  if (giftBtn) giftBtn.onclick = (e) => {
    openGiftToPlayer(profile.uid || targetUid, profile.name, e);
    modal.style.display = 'none';
  };
...
async function renderFriendsList() {
  ...
  list.innerHTML = valid.map(f => `
    <div class="friend-item" onclick="openChatWith('${f.uid}','${f.name}','${f.avatar||'👤'}')">
      ...
      <div style="display:flex;gap:6px">
        <button class="ctx-btn" style="padding:5px 8px;font-size:12px" onclick="event.stopPropagation();openGiftToPlayer('${f.uid}','${f.name}',event)">🎁</button>
...
// Hediye uçuş animasyonu için hedefin ekran konumu — panel varsa onun merkezi, yoksa tıklama noktası
let _giftFlyTarget = null;

function _resolveGiftTargetPoint(targetUid, event) {
  // Oyun içindeyse hedefin panelini bul (uid ya da numeric id ile eşleşebilir)
  const panel = window.players?.find(p => p && (p.uid === targetUid || String(p.id) === String(targetUid)))
    ? document.getElementById(`panel_${window.players.find(p => p && (p.uid === targetUid || String(p.id) === String(targetUid))).id}`)
    : null;
  if (panel) {
  ...
  if (_clickedPlayer && typeof _clickedPlayer.x === 'number') return { x: _clickedPlayer.x, y: _clickedPlayer.y };
  // Hiçbir konum bulunamadıysa ekranın sağ üstüne (bildirim alanına) uç
  return { x: window.innerWidth - 60, y: 90 };
}

window.flyGiftEmoji = function(emoji, targetPoint) {
  const startX = window.innerWidth / 2;
  const startY = window.innerHeight - 80;
  const fly = document.createElement('div');
  fly.className = 'gift-fly-target';
  ...
  requestAnimationFrame(() => {
    fly.style.transform = `translate(${targetPoint.x - startX}px, ${targetPoint.y - startY}px) scale(0.4) rotate(360deg)`;
    fly.style.opacity = '0.2';
  });

  setTimeout(() => {
    fly.remove();
    // Hedefte küçük bir parlama efekti
...
window.openGiftToPlayer = function(targetUid, targetName, event) {
  document.getElementById('playerContextMenu')?.remove();
  _currentChatUid  = targetUid; // hediye gönderirken de aynı hedef
  _currentChatName = targetName;
  _giftFlyTarget = _resolveGiftTargetPoint(targetUid, event);
...
window.sendGiftToPlayer = async function(giftId, targetUid, targetName) {
  ...
  // Animasyon — hediye hedef oyuncunun panelinin/konumunun üzerine uçar
  const panel = document.getElementById('giftPanel');
  if (panel) panel.style.display = 'none';
  const targetPoint = _giftFlyTarget || _resolveGiftTargetPoint(targetUid);
...
window.initSocialSystem = function() {
  ...
  function waitAndInit(attempts) {
    if (window._fb && window._firebaseDb) {
      listenFriendRequests();
      listenNotifications();
    } else if (attempts < 30) {
      setTimeout(() => waitAndInit(attempts + 1), 200);
    }
  }
  waitAndInit(0);
};
